...
def fitness_competition(strategy, opponents, simulations):
    wins = 0
...
def evolve_population(population, fitness_func, mutation_rate, elitisism, simulations):
...
def genetic_algorithm(generations=30, population_size=20, mutation_rate=0.1, simulations=30, elitisism=2, competition=1):
    history = {}
    population = [create_individual() for _ in range(population_size)]
    
    for generation in range(generations):
        if competition:
            # fitness_func = lambda individual, sims: fitness_competition(individual, random.sample(population, min(len(population), competition)), sims)
        else:
            fitness_func = fitness_random
        
        population, (best_fitness, best_individual) = evolve_population(population, fitness_func, mutation_rate, elitisism, simulations)
